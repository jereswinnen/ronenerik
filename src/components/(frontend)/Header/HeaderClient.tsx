  ...
  return (
    <>
      ...
      <header className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-2rem)] max-w-3xl">
        <div className="flex items-center justify-between rounded-2xl bg-c-background/80 backdrop-blur-xl border border-c-foreground/10 px-4 py-2.5">
          ...
          {/* Logo */}
          <Link href="/" className="absolute left-1/2 -translate-x-1/2 font-bold text-lg">
      ...
      {isOpen && (
        <div
          className={`fixed inset-0 z-40 bg-c-background/95 backdrop-blur-lg transition-opacity duration-300 ease-in-out ${
            isAnimating ? 'opacity-100' : 'opacity-0'
          }`}
        >
          <div
            ref={menuRef}
            className={`h-full overflow-y-auto pt-24 pb-8 transition-all duration-300 ease-out ${
          ...
          >
            <div className="container">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Column 1: Podcast */}
                <div
                  className={`rounded-xl border border-c-foreground/10 bg-c-foreground/5 p-5 transition-all duration-500 ease-out ${
                    isAnimating ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
                  }`}
                  style={{ transitionDelay: '100ms' }}
                >
                  <span className="text-xs font-medium uppercase tracking-wider text-c-foreground/60 mb-4 block">
                    Podcast
                  </span>
                  ...
                  {latestEpisode && (
                    <Link
                      ...
                      className="block rounded-lg border border-c-accent/40 overflow-hidden mb-4"
                    >
                      {episodeImage ? (
                        <Media
                ...
                >
                  ...
                  {patreonUrl && (
                    <a
                      ...
                      className="block rounded-xl bg-gradient-to-br from-c-accent/80 to-c-accent/40 p-5"
                    >
